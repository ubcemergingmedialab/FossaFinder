...
public class CPC_PathInspector : Editor
{
    ...
    private float time;
    private CPC_EManipulationModes waypointTranslateMode;
    private CPC_EManipulationModes waypointRotationMode;
    ...
    private GUIContent pauseButtonContent = new GUIContent("Pause", "Paused path follower at current position");
    ...
    private GUIContent relocateContent = new GUIContent("Relocate", "Relocates the path follower to the current view camera's position/rotation");
    ...
    private SerializedProperty pathFollowerGameObjectProperty; // 
    // private SerializedProperty useMainCameraProperty;
    // private SerializedProperty selectedCameraProperty;
    // private SerializedProperty cameraRigProperty; //
    ...
    {
        waypointTranslateMode = (CPC_EManipulationModes)PlayerPrefs.GetInt("CPC_cameraTranslateMode", 1);
        waypointRotationMode = (CPC_EManipulationModes)PlayerPrefs.GetInt("CPC_cameraRotationMode", 1);
    ...
    void GetVariableProperties()
    {
        ...
        pathFollowerGameObjectProperty = serializedObjectTarget.FindProperty("pathFollower"); // 
        ...
        // cameraRigProperty = serializedObjectTarget.FindProperty("cameraRig"); //
    ...
    {
        GUILayout.BeginHorizontal();
        GUILayout.Label("Path follower", GUILayout.Width(Screen.width / 3f));
        pathFollowerGameObjectProperty.objectReferenceValue = (GameObject)EditorGUILayout.ObjectField(pathFollowerGameObjectProperty.objectReferenceValue, typeof(GameObject), true);
        GUILayout.EndHorizontal();
        ...
        //GUILayout.BeginHorizontal();
        //cameraRigProperty.boolValue = GUILayout.Toggle(cameraRigProperty.boolValue, "Use camera rig", GUILayout.Width(Screen.width / 3f));
        //GUI.enabled = cameraRigProperty.boolValue;
        //cameraRigProperty.objectReferenceValue = (GameObject)EditorGUILayout.ObjectField(cameraRigProperty.objectReferenceValue, typeof(GameObject), true);
        //GUI.enabled = true;
        //GUILayout.EndHorizontal();
    ...
    void DrawManipulationDropdown()
    {
        ...
        {
            GUILayout.BeginVertical("Box");
            waypointTranslateMode = (CPC_EManipulationModes)EditorGUILayout.EnumPopup("Waypoint Translation", waypointTranslateMode);
            waypointRotationMode = (CPC_EManipulationModes)EditorGUILayout.EnumPopup("Waypoint Rotation", waypointRotationMode);
    ...
    void DrawWaypointHandles(int i)
    {
        if (Tools.current == Tool.Move)
        {
            ...
            if (waypointTranslateMode == CPC_EManipulationModes.SelectAndTransform)
            {
        ...
        else if (Tools.current == Tool.Rotate)
        {
            ...
            if (waypointRotationMode == CPC_EManipulationModes.SelectAndTransform)
            {
    ...
    void DrawSelectionHandles(int i)
    {
        ...
        {
            if (waypointTranslateMode == CPC_EManipulationModes.SelectAndTransform && Tools.current == Tool.Move
                || waypointRotationMode == CPC_EManipulationModes.SelectAndTransform && Tools.current == Tool.Rotate)
            {
