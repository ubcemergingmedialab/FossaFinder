...
public class CPC_Path : MonoBehaviour
{
    ...
    /// Plays the path
    /// </summary>
    /// <param name="time">The time in seconds how long the path follower takes for the entire path</param>
    ...
    /// <summary>
    /// Pauses the path follower's movement - resumable with ResumePath()
    /// </summary>
    ...
    /// <summary>
    /// When index/time are set while the path is not playing, this method will teleport the path follower to the position/rotation specified
    /// </summary>
    public void RefreshTransform() // 
    {
        pathFollower.transform.position = GetBezierPosition(currentWaypointIndex, currentTimeInWaypoint);
        if (!lookAtTarget)
            pathFollower.transform.rotation = GetLerpRotation(currentWaypointIndex, currentTimeInWaypoint);
        else
            pathFollower.transform.rotation = Quaternion.LookRotation((target.transform.position - pathFollower.transform.position).normalized);
    }
